...
public class MagnetSpawnerScript : MonoBehaviour
{
    ...
    public GameObject parentMech;
    public MechActionsScript mechScript;
    void Start()
    {
        timer = reloadTime;
    ...
    private void initializeMagnet()
    {
        ...
        myMagnetProjectileScript.player = parentMech;
        myMagnet.SetActive(false);
    }
    private void FixedUpdate()
    {
        timer = timer + Time.fixedDeltaTime;
        if (mechScript.recoverMagnet && magnetActive)
        {
            DisableShooting();
    ...
    public void Launch()
    {
        if (parentMech == null)
        {
            Debug.Log("The parent Mech could not be found");
            return;
        }
